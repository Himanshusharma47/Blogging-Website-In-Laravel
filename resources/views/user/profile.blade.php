    ...
    <div class="bg-white py-6 sm:py-8 lg:py-12">
        <div class="mx-auto max-w-screen-2xl px-4 md:px-8">
            <div class="rounded-lg bg-gray-100 px-4 py-6 md:py-8 lg:py-12">
                <p class="mb-2 text-center font-semibold text-indigo-500 md:mb-3 lg:text-2xl">Welcome {{ Auth::user()->name }}
                </p>
                <h2 class="mb-4 text-center text-2xl font-bold text-gray-800 md:mb-6 lg:text-3xl">Here is Your Created Post
                    Available</h2>
            </div>
        </div>
    </div>
                ...
                @foreach ($post as $item)
                    <div class="sm:w-1/2 mb-10 px-4">
                        ...
                        <div class="p-2 w-full">
                            <a href="{{ url('postid/' . $item->id) }}"><button type="submit"
                                    class="flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">Read
                                    More -></button></a>
                        </div>
                    </div>
                @endforeach
